...
The app contains a landing page that displays a distribution which conveys sales in various countries in a Year over Year (YoY) change comparison, a trend chart which shows quarterly sales trend coloured by the different countries and a bar chart showing the top five products in the top ranking country by default. There are also statistics tiles, a table and a map which provides more details about the different segments driving growth. From the dropdown filters on the left, the user can select the start year, end year, country and product of interest. There is also an option to collapse the sidebar to allow the user to view the key metrics in full screen. These filters can also be reset to return the defualt view. 

Furthermore, hovering over the charts displays detailed tooltips with the exact values. The legend items can also be clicked to toggle the visibility of specific countries/products. Our colour palette for categorical comaprison is color-blind friendlyand users can view the dashboard efficiently in a Z-motion. 